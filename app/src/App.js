...
import TipPerson from './components/tipPerson/tipPerson';
import TotalPerson from './components/totalPerson/totalPerson';
...
function App() {
	...
	function handleTipChange(event) {
		...
		// Set state
		setPercent({...percent, selected: value, custom: ''});
	}

	function handleCustomTipChange(event) {
		var checkbox = document.getElementsByClassName("active");

		if(checkbox.length > 0) {
			checkbox[0].children[0].checked = false;
			checkbox[0].classList.remove('active');
		}

		setPercent({...percent, selected: event.target.valueAsNumber, custom: event.target.value})
	}
	...
	function handleReset(){
		...
		setBill('');
		setPeople('');
		setPercent({...percent, selected: '', custom: ''});
			...
			<div className="container">
				...
				<div id="calculator">
					<div className="calculator__inputs">
						<NumberInput id="tip" value={bill} label="Bill" placeholder="0.00" onChange={(event) => setBill(event.target.value)}/>
						<p>Select tip %</p>
						<div className="tip-percent">
							{percent.percentages.map(percent => {
								return <TipPercent
											key={percent}
											className="checkbox" 
											percent={percent} 
											onChange={(event) => handleTipChange(event)}
										/>
							})}
							<NumberInput 
								id="custom" 
								value={percent.custom} 
								label="" placeholder="Custom" 
								onChange={(event) => handleCustomTipChange(event)}
							/>
						</div>
						<NumberInput 
							id="people" 
					...
					<div className="calculator__outputs">
						<TipPerson percent={percent.selected} bill={bill} people={people} />
						<TotalPerson percent={percent.selected} bill={bill} people={people} />
